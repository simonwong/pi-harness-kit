...
export const renderActivityLine = (
  theme: {
    fg: (color: "error" | "success" | "text", text: string) => string;
  },
  ...
  const evidence = input.evidence.length > 0 ? ` · ${input.evidence}` : "";
  let suffix = "";
  if (input.pending) {
    suffix = " …";
  } else if (input.error !== undefined && input.error.length > 0) {
    suffix = ` · ${input.error}`;
  }
  return `${dot} ${input.title}${evidence}${suffix}`;
};
